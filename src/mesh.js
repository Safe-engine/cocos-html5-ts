function createSimpleMeshNode() {
  cc.SimpleMeshNode = cc.Node.extend({
    ...
    _updateContentSizeFromVertices: function () {
      ...
      for (var i = 0; i < this._vertices.length; i += 2) {
        var x = this._vertices[i], y = this._vertices[i + 1];
        if (x < minX) minX = x;
        if (y < minY) minY = y;
        if (x > maxX) maxX = x;
        if (y > maxY) maxY = y;
    ...
    _computeModelMatrixFromNode: function () {
      ...
      var worldPos = this.getParent() ? this.getParent().convertToWorldSpaceAR(cc.p(this.getPosition())) : this.getPosition();
      ...
      var worldRotation = this.getNodeToWorldTransform ? (function () {
        // use getNodeToWorldTransform to extract rotation/scale if available; else fallback to this.getRotation()
        try {
          var t = this.getNodeToWorldTransform(); // returns affine transform with a, b, c, d, tx, ty
      ...
      if (worldRotation) {
        ...
        var worldPt = this.convertToWorldSpaceAR(cc.p(0, 0));
        tx = worldPt.x; ty = worldPt.y;
      ...
      var rad = (rotDeg || 0) * Math.PI / 180.0;
      ...
      var anchor = this.getAnchorPoint ? this.getAnchorPoint() : { x: 0, y: 0 };
      var aw = anchor.x * this.width;
      var ah = anchor.y * this.height;
      // compute e,f
      var e = tx - (a * aw + c * ah);
      var f = ty - (b * aw + d * ah);
    ...
    _drawMeshWebGL: function () {
      ...
      var aPosLoc = gl.getAttribLocation(this._program, 'a_position');
      var aUVLoc = gl.getAttribLocation(this._program, 'a_texcoord');
      ...
      if (this._texture) {
        ...
        var webTex = this._texture._webTextureObj || (this._texture.getTexture ? this._texture.getTexture() : null);
        if (webTex) {
          gl.bindTexture(gl.TEXTURE_2D, webTex._webTextureObj);
        } else {
          // fallback: try html image object
          var img = this._texture.getHtmlElementObj ? this._texture.getHtmlElementObj() : this._texture;
          if (img) {
        ...
        var uTexLoc = gl.getUniformLocation(this._program, 'u_texture');
      ...
      // resolution
      var uResLoc = gl.getUniformLocation(this._program, 'u_resolution');
      var sz = cc.view.getFrameSize ? cc.view.getFrameSize() : { width: cc.director.getWinSize().width, height: cc.director.getWinSize().height };
      gl.uniform2f(uResLoc, sz.width, sz.height);

      // model matrix from node
      var uModelLoc = gl.getUniformLocation(this._program, 'u_model');
      var modelMat = this._computeModelMatrixFromNode();
      cc.glUseProgram(this._program)
      gl.uniformMatrix3fv(uModelLoc, false, modelMat);

      // alpha
      var uAlphaLoc = gl.getUniformLocation(this._program, 'u_alpha');
    ...
    _drawMeshCanvasFallback: function () {
      ...
      var rect = [cc.p(minX, minY), cc.p(maxX, minY), cc.p(maxX, maxY), cc.p(minX, maxY)];
  ...
  cc.SimpleMeshNode.createQuad = function (x, y, w, h, u0, v0, uw, vh) {
    var verts = new Float32Array([x, y, x + w, y, x + w, y + h, x, y + h]);
    var u1 = u0 + uw, v1 = v0 + vh;
    var uvs = new Float32Array([u0, v0, u1, v0, u1, v1, u0, v1]);
